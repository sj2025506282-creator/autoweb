...
import type { Restaurant, AnalyticsStats } from "@autoweb/shared";
...
export default async function AnalyticsPage({
  ...
  const stats = await apiFetch<AnalyticsStats>('/api/analytics/' + id + '/stats');
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white rounded-lg shadow-sm border p-6">
          <TopList title="Top Pages" items={stats.topPages as unknown as TopItem[]} keyField="page" />
        </div>
        <div className="bg-white rounded-lg shadow-sm border p-6">
          <TopList
            title="Top Referrers"
            items={stats.topReferrers as unknown as TopItem[]}
            keyField="referrer"
          />
        </div>
      </div>
